...
# EmailJS details (replace if needed)
...
def get_random_paragraph_from_docs():
    ...
    if not paragraphs:
        # raise Exception("לא נמצאו פסקאות במסמך. ודא שהתוכן קיים ושהקישור פומבי.")
        raise Exception("no sections were founds")
...
def upload_to_github(html_content):
    ...
    if put_resp.status_code in [200, 201]:
        ...
        print("Page landed on github successfully")
    else:
        #print("❌ שגיאה בהעלאה ל-GitHub:", put_resp.json())
        print("Error uploading to github", put_resp.json())
